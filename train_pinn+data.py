...
class AdaptiveSampler:
    def __init__(self, label_fig, label_press, pinn_fig):
        """
        :param label_fig:
    ...
    def sample_pinn(self, net, sample_num, p_last):
        """
        select new samples by residual
        :param net:
        :param sample_num:
        :return:
        """
        X = self.pinn_fig.to(selected_gpu)
        net.eval()
        # p_pre = net(X)
        p_next = net(X) * (max_pt - min_pt) + min_pt

        res = torch.zeros_like(p_next)
        res[:] = res[:] + (cell_volume * porosity * ct / dt) * (p_next[:] - p_last[:])
        trans = X.view(-1, 4 * nx * ny * nz)
        for d in range(4):
            res[:] = (res[:] - scaled_params * trans[:, d * 400:(d + 1) * 400] *
                    (p_next[:, neighbor_idx[d]] - p_next[:]))

        res[:, well_mark] += PI * (p_next[:, well_mark] - pwf)
        # res[:, bc_index] += PI * (p_next[:, bc_index] - pwf)
        error = res.sum(dim=1)
        values, indices = torch.topk(error, sample_num)
        new_index = [i for i in range(len(X)) if i not in indices]
        # update set
        self.pinn_fig = X[new_index]
        # self.pinn_press = p_next[new_index]
        return X[indices], p_next[indices]
...
def data_prepare(args):
    ...
    if args.flag:
        pinn_fig_set = train_trans_matrix[:pinn_size] / scaled_params
    else:
        pinn_fig_set = train_trans_matrix[label_size:label_size + pinn_size] / scaled_params
    ...
    params: dict[str, float | torch.Tensor] = {'pinn_set': pinn_fig_set, 'max_pt': max_pt, 'min_pt': min_pt,
...
def train(args, params):
    ...
    for t in range(nt):
        ...
        min_loss = 1e10
        sampler = AdaptiveSampler(train_label_fig, train_label_press, pinn_loader)
        # 第一次加载数据训练, 找到loss最低的模型
        for batch_id, pinn_fig in enumerate(pinn_loader):

            if t == 0:
                p_last = torch.ones((len(pinn_fig), 400)).to(torch.float64).to(selected_gpu)
            else:
                p_last = press_his.data[batch_id]

            input_fig = pinn_fig.to(selected_gpu)
            trans = input_fig.view(-1, 4 * nz * ny * nx)

            for i in tqdm(range(max_iter), desc='training'):
                pinn_loss = compute_pinn_loss(layer_t, optimizer_t, criterion, input_fig, trans, p_last)
                label_loss = compute_label_loss(layer_t, optimizer_t, criterion, train_label_fig,
                                                train_label_press[:, t, :])
                ...
                if (i + 1) % interval == 0:
                    # layer_t.eval()
                    # p_pre = layer_t(train_label_fig) * (max_pt - min_pt) + min_pt
                    # loss2 = criterion(p_pre / p_init, train_label_press[:, t, :] / p_init)  # .mean(dim=0).sum()
                    ...
                    writer.add_scalars('PINN & label loss',
                                       {'pinn loss': loss.item(), 'label loss': label_loss}, global_step=gs)
        ...
        for batch_id, pinn_fig in enumerate(pinn_loader):
            ...
            press_his.update(best_p_next.detach(), batch_id)
...
parser.add_argument('-g', '--logdir', default='./logs/pinn800+label10_4well')
...
parser.add_argument('--bs', default=1600, type=int, help='batch size')
parser.add_argument('--lr', default=3e-3, type=float, help='learning rate')
parser.add_argument('--interval', default=20, type=int)
parser.add_argument('-mn', '--model_name', default='pinn1600+label800_4well', type=str)
# 3: 使用全局最大最小值训练； 4: 使用p_scale 训练；
parser.add_argument('-ps', '--pinn_size', default=800, type=int)
parser.add_argument('-ls', '--label_size', default=100, type=int)
parser.add_argument('--flag', default=False, type=bool,
                    help='whether use pinn of training data')
...
if __name__ == '__main__':
    ...
    pinn_loader, test_fig, test_press = params['pinn_loader'], params['test_fig'], params['test_press']
    ################# NN params ##############
