...
if __name__ == '__main__':
    ...
    batch_size = min(args.bs, args.label_size)
    # 1. load data
    ...
    min_label_pt = torch.min(torch.tensor(train_press), dim=0).values
    max_label_pt = torch.max(torch.tensor(train_press), dim=0).values
    ...
    train_label_press = torch.tensor(train_press[train_label_index], dtype=torch.float64).to(selected_gpu)
    ...
    if args.pretrain is not None:
        ...
        model = CNN(output_size).to(torch.float64).to(selected_gpu)
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=args.max_iter, eta_min=1e-3)
    # train model
    nt = args.nt
    ...
    for t in range(nt):
        ...
        for i in tqdm(range(max_iter), desc='training'):
            ...
            loss = criterion(p_pre / p_init, train_label_press[:, t, :] / p_init)  # .mean(dim=0).sum()
            ...
            if (i + 1) % interval == 0:
                print('\ntime step:{}, train step:{:d}, label loss:{}'.format(t, i + 1, loss))
                print('now, the minimum loss at {}th iter, and min_loss is{}'.format(min_iter_time, min_loss))

                model.eval()
                test_p = model(test_fig).detach() * (max_pt - min_pt) + min_pt
                # 应该计算下一个时间步的压力，
                label_t_press = torch.tensor(test_press[:, t, :] / p_init).to(selected_gpu)
                current_L1 = (L1_loss(test_p / p_init, label_t_press) / label_t_press.abs()).mean()
                writer.add_scalar('label loss', loss, global_step=gs)
                ...
                test_loss_b = L1_loss(test_p_b / p_init, label_t_press) / label_t_press.abs()
                re_L1_b = test_loss_b.mean()
                print('current model, test dataset L1 relative error: {}'.format(current_L1.item()))
        ...
        best_p_next = best_model(train_label_fig) * (max_pt - min_pt) + min_pt
        ...
        plot_10_figs(best_p_next[rand_idx, :], nx, ny,
                     args.fig_dir + 'train_' + args.model_name + 'sample_t_{}.png'.format(t + 1), id_num=rand_idx)
    ...
    for t in range(nt):
        ...
        if (t + 1) % 10 == 0:
            ...
            plot_10_figs(vari_press[t - 9: t + 1], nx, ny,
                         args.fig_dir + 'vari_' + args.model_name + '_t_{}.png'.format(t + 1), show=False)
