...
class AdaptiveSampler:
    ...
    def sample_label(self, net, sample_num, criterion):
        """
    ...
    def sample_pinn(self, net, sample_num, model_list):
        """
        select new samples by residual
        :param net:
        :param sample_num:
        ...
        if len(model_list) > 0:
            layer_last = model_list[-1]
            p_last = layer_last(X).detach() * (max_pt - min_pt) + min_pt
        else:
            p_last = torch.zeros_like(p_next).to(torch.float64).to(selected_gpu)

        res = torch.zeros_like(p_next)
        res[:] = res[:] + (cell_volume * porosity * ct / dt) * (p_next[:] - p_last[:])
        # 把trans_matrix 改成4个方向的tensor可以加速。
        trans = X.view(-1, 4*nz*nx*ny)
        for d in range(4):
        ...
        # return P_last of new samples
        return X[indices], p_last[indices]
...
def train(args, params):
    ...
    for t in range(nt):
        ...
        min_loss = 1e10
        sampler = AdaptiveSampler(label_set_pair['label_fig'], label_set_pair['label_press'][:, t, :], pinn_set)
        ...
        for i in tqdm(range(max_iter), desc='training'):
            ...
            if (i + 1) % interval == 0:
                ...
                if args.AS:
                    ...
                    if j <= 4:
                        X, y = sampler.sample_label(layer_t, sample_label_list[j - 1], criterion=L1_loss)
                        # X, y = adaptive_sampler.sample(layer_t, 10, criterion=L1_loss)
                        label_fig = torch.concat((label_fig, X), dim=0)
                        label_press = torch.concat((label_press, y), dim=0)
                        print(f'using adaptive sampling, j={j}, label_fig length: {len(label_fig)}')  # 最后一步不需要采样
                        X_p, y_p = sampler.sample_pinn(layer_t, sample_pinn_list[j-1], new_model_list)
                        pinn_fig = torch.concat((pinn_fig, X_p), dim=0)
                        p_last = torch.concat((p_last, y_p), dim=0)
                        print(f'using adaptive sampling, j={j}, pinn_fig length: {len(pinn_fig)}')
...
parser.add_argument('-g', '--logdir', default='./logs/AS400_pinn+data_4well')
parser.add_argument('--fig_dir', default='./label400_4well/', type=str)
...
parser.add_argument('-ls', '--label_size', default=400, type=int)
