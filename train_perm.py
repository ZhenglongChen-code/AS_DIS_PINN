...
def data_prepare(args):
    """
    prepare data for training, in this file using permeability as training data
    :param args:
    :return:
    """
    ...
    # training fig
    train_label_fig = torch.tensor(train_permeability[train_label_index]).reshape(-1, nz, ny, nx).to(selected_gpu) / scaled_params
    train_label_press = train_press[train_label_index].to(selected_gpu)
    train_trans_fig = train_trans_matrix[train_label_index].clone().to(selected_gpu) / scaled_params
    ...
    test_fig = torch.tensor(test_permeability).reshape(-1, nz, ny, nx).to(selected_gpu) / scaled_params
    ...
    params: dict[str, float | torch.Tensor] = {'label_pair': label_pair, 'max_pt': max_pt, 'min_pt': min_pt,
                                               ...
                                               'train_trans_fig': train_trans_fig * scaled_params,
...
def train(args, params):
    ...
    train_trans_fig = params['train_trans_fig']
    ...
    for t in range(nt):
        ...
        adaptive_sampler = AdaptiveSampler(params['label_pair']['label_fig'],
                                           params['label_pair']['label_press'][:, t, :])
...
parser.add_argument('-g', '--logdir', default='./logs/trans100')
...
parser.add_argument('-mn', '--model_name', default='trans100', type=str)
